...
async def update_usuario(id: int, usuario: schemas.schema_usuario.UsuarioUpdate, db: Session = Depends(get_db), token: str = Depends(auth.oauth2_scheme)):
...
async def delete_usuario(id: int, db: Session = Depends(get_db), token: str = Depends(auth.oauth2_scheme)):
